...
export default function Login () {
   ...
   const [nameIsTouched, setNameIsTouched] = useState(false)
    ...
    return(
        <div class=" block ml-100
        . p-6 rounded-lg shadow-lg bg-white max-w-sm">
  <form onSubmit={formSubmissionHandler}>
    <div class="form-group mb-6 ">
      <label class="form-label inline-block mb-2 text-gray-700">Nom</label>
      <input type="text" class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300
    ...
    <button type="submit" class="
      ...
      active:bg-red-800 active:shadow-lg transition duration-150 ease-in-out">Submit</button>
      {nameState}
      <button class="
      px-6 py-2.5 bg-red-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-red-700 focus:shadow-lg focus:outline-none focus:ring-0
      active:bg-red-800 active:shadow-lg transition duration-150 ease-in-out"
      onClick={changePage}>Home</button>
  </form>
</div>     
    )
}